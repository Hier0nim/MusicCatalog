...
public class AlbumController : Controller
{
    ...
    [HttpPost]
    public IActionResult Index(int pageSize, int? pageNumber, string titleSearchString, string artistSearchString,
        int yearSearchNumber)
    {
        if (!pageNumber.HasValue) pageNumber = 1;
        titleSearchString ??= string.Empty;
        var sortOrder = TempData.ContainsKey("albumSortOrder") ? (string)TempData.Peek("albumSortOrder") : "";
        var userId = User.FindFirstValue(ClaimTypes.NameIdentifier);
        var model = _albumService.GetAlbumsFromUserForList(pageSize, pageNumber.Value, titleSearchString,
            artistSearchString, yearSearchNumber, userId, sortOrder);
        return View(model);
    }
